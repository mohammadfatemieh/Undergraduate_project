...
delay_nms(1);
EX1=1;//开启外部中断
if(flag2==1) //将所测的时间数转换成距离
{ //
temp=high_time2*256+low_time2;
temp=(temp/1000)/2;
temp*=340;
temp=temp/10;
dis2=(unsigned int)temp;//dis将直接用于显示函数
flag2=0;
}
}
...
temp*=340;
temp=temp/10;
dis3=(unsigned int)temp;//dis将直接用于仙四函数
flag1=0;
}
}
////////////////4接收共2csb
...
temp*=340;
temp=temp/10;
dis4=(unsigned int)temp;//dis将直接用于显示函数
flag2=0;
}
}
/**********************************
...
**********************************/
void TT() interrupt 0//中断号为0时对应中断0
{
uint tmp;
TR0=0;//关闭定时器；
ET0=0;//关闭定时器允许位
...
//////外部中断一
void TT1() interrupt 2//中断号为2时对应中断1
{
uint tmp;
TR1=0;//关闭定时器1；
ET1=0;//关闭定时器允许位1
flag2=1; //标志位置唯一
tmp=TH1*256+TL1; //对计数器数值的提取
if((tmp>0)&&(tmp<60000))//判断所测得计时值是否在量程类
{
high_time2=TH1;//对时间的提取
low_time2=TL1;
